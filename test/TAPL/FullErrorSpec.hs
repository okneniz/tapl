...
module TAPL.FullErrorSpec where

import Test.Hspec
import TAPL.FullError.Evaluator (eval)
        ...
        describe "assignment and let" $ do
            let test = (\(x,y) -> it x $ do { eval x "<stdin>" `shouldBe` Right y })
                examples = [
                  ...
                  ("let x = ref error in x", "let x = ref error in x:Ref Bot")
                 ]
            mapM_ test examples
