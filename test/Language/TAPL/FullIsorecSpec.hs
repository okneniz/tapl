    ...
    describe "operations" $ do
        ...
        describe "oop" $ do
            describe "Counter" $ do
                ...
                \ let create = fix (lambda cr: {x:Nat}->Counter. \
                \   (lambda s: {x:Nat}.fold [Counter] {get=s.x, inc=(lambda u:Unit. cr {x=succ(s.x)})}) \
                \ ) in \
                \ let p = create {x=zero} in "

                tests evalString [
                      (
                        definitions ++ "let c = unfold [Counter] p in c.get",
                        pass "zero:Nat"
                      ),
                      (
                        definitions ++ "let c = unfold [Counter] p in c.inc unit",
                        pass "fold [Counter] {get=succ zero, inc=(lambda u.(lambda cr.(lambda s.fold [Counter] {get=s.x, inc=(lambda u'.cr {x=succ s.x})})) {x=succ {x=succ zero}.x})}:Counter"
                      ),
                      (
                        definitions ++ "let c = unfold [Counter] p in \
                        \ let cc = c.inc unit in \
                        \ let pcc = unfold [Counter] cc in \
                        \ pcc.get",
                        pass "succ zero:Nat"
                      )
                 ]
