    ...
    describe "values" $ do
        ...
        describe "abstractions" $ do
            tests evalString [
                    ...
                    ("(lambda x.x zero);", pass "(lambda x'.x' zero):((Nat -> x1) -> x1)"),
                    ("(lambda x:X.lambda y:X->X.y x)", pass "(lambda x'.(lambda y'.y' x')):(X -> ((X -> X) -> X))"),

                    ("(lambda x:X.x)", pass "(lambda x'.x'):(X -> X)"),
                    ("(lambda z:Z.(lambda y:Y.z (y true)))", pass "(lambda z'.(lambda y'.z' y' true)):((x0 -> x1) -> ((Bool -> x0) -> x1))"),
                    (
                      "(lambda w:W.if true then false else w false)",
                      pass "(lambda w'.if true then false else w' false):((Bool -> Bool) -> Bool)"
                    )
                 ]
