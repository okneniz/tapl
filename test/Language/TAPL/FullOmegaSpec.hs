module Language.TAPL.FullOmegaSpec where
import Test.Hspec
import Language.TAPL.TestHelpers
import Language.TAPL.FullOmega.Evaluator (evalString)
        ...
        describe "Poly" $ do
           tests evalString [
                    ...
                    (
                      ...
                      pass "(lambda x.x):((All X::*. (X -> T)) -> (All X::*. (X -> T)))"
                    ),
                    (
                    ...
                    (
                      ...
                      pass "{*(All Y::*. Y), (lambda x.x)}:{Some X::*, (X -> X)}"
                    ),
                    (
                      "{*Nat, {c=zero, f=(lambda x:Nat. succ x)}} as {Some X, {c:X, f:X->Nat}}",
                      pass "{*Nat, {c=zero, f=(lambda x.succ x)}}:{Some X::*, {c=X, f=(X -> Nat)}}"
                    ),
                    (
        ...
        describe "Omega" $ do
            describe "Pair" $ do
                ...
                tests evalString [
                    (
                      definitions ++ "f",
                      pass "(lambda X.(lambda Y.(lambda f.f))):(All X::*. (All Y::*. (Pair X Y -> Pair X Y)))"
                    ),
                    (
                    ...
                    (
                      definitions ++ "pair",
                      pass "(lambda X.(lambda Y.(lambda x.(lambda y.(lambda R.(lambda p.p x y)))))):(All X::*. (All Y::*. (X -> (Y -> (All R::*. ((X -> (Y -> R)) -> R))))))"
                    ),
                    (
                      definitions ++ "pair [Nat] [Bool] zero false",
                      pass "(lambda R.(lambda p.p zero false)):(All R::*. ((Nat -> (Bool -> R)) -> R))"
                    ),
                    (
                      definitions ++ "\
            ...
            describe "List" $ do
                ...
                tests evalString [
                    (
                      definitions ++ "diverge",
                      pass "(lambda X.(lambda x.(lambda x'.x'))):(All X::*. (Unit -> X))"
                    ),
                    (
                      definitions ++ "nil",
                      pass "(lambda X.(lambda R.(lambda c.(lambda n.n)))):(All X::*. (All R::*. ((X -> (R -> R)) -> (R -> R))))"
                    ),
                    (
                      definitions ++ "cons",
                      pass "(lambda X.(lambda hd.(lambda tl.(lambda R.(lambda c.(lambda n.c hd tl R c n)))))):(All X::*. (X -> (List X -> (All R::*. ((X -> (R -> R)) -> (R -> R))))))"
                    ),
                    (
                      definitions ++ "isnil",
                      pass "(lambda X.(lambda l.l Bool (lambda hd.(lambda tl.false)) true)):(All X::*. (List X -> Bool))"
                    ),
                    (
                      definitions ++ "head",
                      pass "(lambda X.(lambda l.let d = (lambda X'.(lambda x.(lambda x'.x'))) X in let z = (lambda hd.(lambda tl.(lambda z.hd))) in let r = Pair (Unit -> List) l X in l unit)):(All X::*. (List X -> X))"
                    ),
                    (
                      definitions ++ "tail",
                      pass "(lambda X.(lambda l.(lambda X'.(lambda Y.(lambda p.p X' (lambda x.(lambda y.x))))) List X List X l Pair List X List X (lambda hd.(lambda tl.(lambda X'.(lambda Y.(lambda x.(lambda y.(lambda R.(lambda p.p x y)))))) List X List X (lambda X'.(lambda Y.(lambda p.p Y (lambda x.(lambda y.y))))) List X List X tl (lambda X'.(lambda hd'.(lambda tl'.(lambda R.(lambda c.(lambda n.c hd' tl' R c n)))))) X hd (lambda X'.(lambda Y.(lambda p.p Y (lambda x.(lambda y.y))))) List X List X tl)) (lambda X'.(lambda Y.(lambda x.(lambda y.(lambda R.(lambda p.p x y)))))) List X List X (lambda X'.(lambda R.(lambda c.(lambda n.n)))) X (lambda X'.(lambda R.(lambda c.(lambda n.n)))) X)):(All X::*. (List X -> List X))"
                    ),
                    (
                      definitions ++ "cons [Nat]",
                      pass "(lambda hd.(lambda tl.(lambda R.(lambda c.(lambda n.c hd tl R c n))))):(Nat -> (List Nat -> (All R::*. ((Nat -> (R -> R)) -> (R -> R)))))"
                    ),
                    (
                      definitions ++ "\
                      ...
                      \list",
                      pass  "(lambda R.(lambda c.(lambda n.c succ succ succ zero (lambda R'.(lambda c'.(lambda n'.c' succ succ zero (lambda R''.(lambda c''.(lambda n''.c'' succ zero (lambda R'''.(lambda c'''.(lambda n'''.c''' zero (lambda R''''.(lambda c''''.(lambda n''''.n''''))) R''' c''' n'''))) R'' c'' n''))) R' c' n'))) R c n))):(All R::*. ((Nat -> (R -> R)) -> (R -> R)))"
                    ),
                    (
                      definitions ++ "\
                      \let one = succ zero in \
