module Language.TAPL.FullFSubSpec where
import Test.Hspec
import Language.TAPL.TestHelpers
import Language.TAPL.FullFSub.Evaluator (evalString)
        ...
        describe "Poly" $ do
           tests evalString [
                    ...
                    (
                      ...
                      pass "{*(All Y<:Top. Y), (lambda x.x) as {Some X<:Top. (X -> X)}}:{Some X<:Top. (X -> X)}"
                    ),
                    (
                      "{*Nat, {c=zero, f=(lambda x:Nat. succ x)}} as {Some X, {c:X, f:X->Nat}}",
                      pass "{*Nat, {c=zero, f=(lambda x.succ x)} as {Some X<:Top. {c=X, f=(X -> Nat)}}}:{Some X<:Top. {c=X, f=(X -> Nat)}}"
                    ),
                    (
