module Language.TAPL.FullIsorec.Pretty (render, renderType) where
...
prettify (TFold _ ty) = do
    doc1 <- prettifyType ty
    return $ pretty "fold" <+> brackets doc1

prettify (TUnfold _ ty) = do
    doc1 <- prettifyType ty
    return $ pretty "unfold" <+> brackets doc1
