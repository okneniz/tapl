module Language.TAPL.FullOmega.Pretty (render, renderType) where
...
prettify (TRef _ t) = prettyFunc "ref" t
...
prettify (TUnpack _ tyX x t1 t2) = do
     ...
     withTmpStateT (\s -> s { names = addName tyX1 (names s)}) $ do
        nn <- getNames
        let x1 = fst $ pickFreshName nn x
        withTmpStateT (\s -> s { names = addName x1 (names s)}) $ do
            doc1 <- prettify t1
            doc2 <- prettify t2
...
prettifyType (TyRef ty) = prettyTypeFunc "Ref" ty
