module Language.TAPL.FullPoly.Context where
...
bindingShift _ _ TypeVarBind = return TypeVarBind
bindingShift p d (TypeAddBind ty) = TypeAddBind <$> typeShift p d ty
...
typeShiftAbove p d c ty = typeMap onVar c ty
                  where onVar c x n | x >= c = if x + d < 0 -- this is from book
