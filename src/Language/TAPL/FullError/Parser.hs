...
module Language.TAPL.FullError.Parser where
...
import Language.TAPL.FullError.Memory
...
instance ParserContext (LCParserContext Term) where
    ...
    typeOf c@(LCParserContext n s (TLet info v t1 t2)) = do
        ty1 <- typeOf $ LCParserContext n s t1
        let context' = bind c v (VarBind ty1)
        ty2 <- typeOf $ context' `withTerm` t2
        return ty2

    typeOf c@(LCParserContext _ _ (TError _)) = do
        return TyBot

-- так как в 14 главе ещё не реализованны подтипы, то конструкцию try with нельзя будет типизировать
-- поэтому и fullerror пример на сайте не имеет реализации eval для этой конструкции
--
--    typeOf c@(LCParserContext n m (TTry i t1 t2)) = do
...
instance Show (LCParserContext Term) where
    ...
    show (LCParserContext n s (TError _)) = "error"
...
parse code path =
            runParser fullErrorParser pureContext path code
...
notApply = (try assign <?> "assignment")
        ...
        <|> try variable
        <|> parens notApply
--        <|> try try' в 14 главе так всё равно не получится без подтипов
...
topAnnotation = primitiveType "Top" TyBot
